champ.QC_recursive <- function (beta = myLoad$beta, pheno = myLoad$pd$Sample_Group, 
    mdsPlot = TRUE, densityPlot = TRUE, dendrogram = TRUE, PDFplot = TRUE, 
    Rplot = FALSE, Feature.sel = "None", resultsDir = "./CHAMP_QCimages/") 
{
    message("[=====================================]")
    message("[<<<<< ChAMP.QC_recursive START >>>>>>]")
    message("---------------------------------------")
    if (!file.exists(resultsDir))
        dir.create(resultsDir, recursive = TRUE)
    message("champ.QC_recursive results will be saved in ", resultsDir)
    ...
    if (mdsPlot) {
        if (Rplot) 
            mdsPlot(beta, numPositions = 1000, sampGroups = pheno, 
                colnames(beta))
        if (PDFplot) {
            pdf(paste(resultsDir, "raw_mdsPlot.pdf", sep = "/"), 
                width = 6, height = 4)
            mdsPlot(beta, numPositions = 1000, sampGroups = pheno, 
                colnames(beta))
            dev.off()
    ...
    if (densityPlot) {
        if (Rplot) 
            densityPlot(beta, sampGroups = pheno, main = paste("Density plot of raw data (", 
                nrow(beta), " probes)", sep = ""), xlab = "Beta")
        if (PDFplot) {
            pdf(paste(resultsDir, "raw_densityPlot.pdf", sep = "/"), 
                width = 6, height = 4)
            densityPlot(beta, sampGroups = pheno, main = paste("Density plot of raw data (", 
                nrow(beta), " probes)", sep = ""), xlab = "Beta")
            dev.off()
    ...
    if (dendrogram) {
        ...
        dend <- dend %>% set("leaves_pch", 19) %>% set("leaves_cex", 
            0.6) %>% set("leaves_col", MyColor[pheno[order.dendrogram(dend)]])
        if (Rplot) {
            plot(dend, center = TRUE, main = paste("All samples before normalization (", 
                nrow(beta), " probes)", sep = ""))
            legend("topright", fill = MyColor, legend = names(MyColor))
        }
        if (PDFplot) {
            pdf(paste(resultsDir, "raw_SampleCluster.pdf", sep = "/"), 
